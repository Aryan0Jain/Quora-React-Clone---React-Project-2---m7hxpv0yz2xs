...
export default function Spaces() {
	...
	return (
		<div className="pt-[88px] md:pt-14 min-h-screen w-full pb-4">
			<div className="w-full md:w-9/12 flex gap-4">
				{loading || (status === "loading" && <div>Loading</div>)}
				{!loading && status === "authenticated" && (
					<div className="w-full md:w-9/12 mx-auto flex flex-col gap-4">
						<div className="pt-4 bg-white border rounded-sm flex flex-col items-start gap-2 md:mt-7">
							<div className="font-bold text-[18px] px-4">
								Your Spaces
							</div>
							<button
								onClick={() => setShowCreateModal(true)}
								className="border-2 rounded-full flex items-center gap-1 px-2 mx-4 py-1 text-[#2e69ff] border-[#2e69ff] hover:bg-[#ebf0ff] transition"
							>
							...
							{spaces
								...
								.map((space) => {
									return (
										...
										>
											<div className="flex gap-2 items-center w-full border-b py-2 px-4">
						...
						<div className="font-bold text-xl">Discover Spaces</div>
						<div className="font-medium">Spaces you might like</div>
								...
								.map((space, index) => {
									return (
										<Link
											...
											href={`/spaces/${space._id}`}
										>
											<div className="bg-white rounded-2xl w-44 h-56 overflow-hidden text-center">
												<div
				...
				<div className="hidden md:block">Adds</div>
			</div>
		</div>
	);
}
