...
export async function searchData(jwt, query, limited = false) {
	...
	try {
		let data = await fetch(
			`https://academics.newtonschool.co/api/v1/quora/post?search={"title":"${query}","content":"${query}"}&limit=${limited ? 5 : 1000}`,
			{
				method: "GET",
				headers: {
					projectID: projectID,
		...
		data = await fetch(
			`https://academics.newtonschool.co/api/v1/quora/user/?search={"name":"${query}"}&limit=${limited ? 5 : 1000}`,
			{
				method: "GET",
				headers: {
					projectID: projectID,
